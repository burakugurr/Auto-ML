...
def plotpred(df, pred):
    ...
    ax.plot(pred.index, pred['y-pred'].values,
            label='Predict', color='#ff0000')
...
def homepage(username):
    ...
    col2.markdown(
        "**Stock App is a web app that can help you to predict stock prices and anomaly detection.**")
    col2.write("""
...
def datainsgiht():
    ...
    st.markdown(
        """ Please select a stock from the dropdown menu below to see the stock price result""")
    col1, col2 = st.columns(2)
    s_box = col1.selectbox(
        "Select a stock", ("AAPL", "MSFT", "AMZN", "GOOG", "FB", "NFLX", "Other"))
    if s_box == "Other":
        s_w = col1.text_input("Please type a stock name")

    start = col2.date_input("Start Date", value=datetime.date(
        2000, 1, 1), max_value=datetime.date.today())
    end = col2.date_input(
        "End Date", value=datetime.datetime.now(), max_value=datetime.date.today())
        ...
        if(s_box == "Other"):
            if(s_w != ""):
                try:
                    ...
                    st.subheader("Stock Price on "+str(s_w) +
...
def forecasting():
    ...
    method = st.sidebar.selectbox(
        "Select a Forecast method", ("Box-Jenkins", "Neural Network"))

    if(method == "Box-Jenkins"):

        diffnum = st.sidebar.slider(
            "Select a discrete difference of data", 0, 30, 1)
        lagnum = st.sidebar.slider(
            "Select lagg discrete difference of data", 0, 100, 1)
        ...
        if(df_diff is False):
            ...
            col1, col2 = st.columns(2)
            col1.pyplot(plot_acf(df_diff.dropna(), lags=lagnum, title="Stock Price ACF"),
                        color="green", vlines_kwargs={"color": "green"})
            col2.pyplot(plot_pacf(df_diff.dropna(),
                        lags=lagnum, title="Stock Price PACF"))
        ...
        method_name = st.sidebar.multiselect("Select a evulation metrics", [
                                             "MAPE", "RMSE", "MAE", "MSE", "R2"], default=["MAPE"])
        ...
        if(issesion):
            ...
            if st.sidebar.button("Forecast"):
                pred, model = fc.arima_model(df['Adj Close'][:n_size], int(
                    p), int(d), int(q), int(P), int(D), int(Q))
                col1, col2 = st.columns(2)
                col1.write(pred)
                ...
                try:
                    for i in range(len(result)):
                        col1.metric(list(result.keys())[
                                    i], list(result.values())[i])
                        col2.metric(list(result.keys())[
                                    i+1], list(result.values())[i+1])
                        col3.metric(list(result.keys())[
                                    i+2], list(result.values())[i+2])
                        col4.metric(list(result.keys())[
                                    i+3], list(result.values())[i+3])
                        col5.metric(list(result.keys())[
                                    i+4], list(result.values())[i+4])
                        break
                except:
                    pass
        ...
        # Sessional olmadığı zaman
            ...
            pred, predmodel = fc.arima_model(df['Adj Close'][:n_size], int(
                p), int(d), int(q), P=None, D=None, Q=None)
            ...
            if(pred_feature):
                date_pred_start = st.date_input(
                    "Select a start date to predict", min_value=df.index.max())
                date_pred_end = st.date_input(
                    "Select a end date to predict", min_value=date_pred_start)
                if st.button("Predict"):
                    pred = Forecast.predict_feature(
                        model=predmodel, start=date_pred_start, end=date_pred_end)
        ...
        model_methot = st.sidebar.selectbox(
            "Select a Neural Network Model", ("LSTM", "RNN"))
        ...
        if(model_methot == "LSTM"):
            ...
            window_size = st.sidebar.slider(
                "Select a size of window size", 1, train_size, 2)

        loss_func = st.sidebar.radio(
            "Select a loss function", ("MSE", "MAE", "MSLE", "MAPE"))
        optimizer_methot = st.sidebar.radio(
            "Select a optimizer", ("SGD", "RMSprop", "Adam", "Adagrad", "Adadelta", "Adamax", "Nadam"))
    ...
    # =============================================================================
                ...
                with st.spinner('Wait for result...'):
                    pred_df_test, pred_df_train = Forecast.RNN(
                        df, train_size, window_size, loss_func, optimizer_methot, epoch_size)
                    if(pred_df_test is not None):
                        st.success('Done!')
                st.caption("Prediction Result")
                st.plotly_chart(plot_nn(pred_df_train, pred_df_test,
                                df, "Stock Price Prediction with RNN"))
    ...
    # =============================================================================
                ...
                for i in method_name:
                    ...
                    values = evulation(
                        i, pred_df_test['test_pred'], df['Adj Close'][len(pred_df_train):])
                    metricsList.append(i)
                    valueList.append(values)
...
def anomaly():
    ...
    fraction = st.sidebar.slider(
        "Select the number of fraction", 0.0, max_value=1.0, value=0.5, step=0.01)
...
def aboutpage():
    ...
    *********************************
    ...
    """)
    st.markdown(
        """**If you like this project, please give a star on Github. Thank you!** """)
...
def app(db, user, username="User"):
    ...
    app_mode = st.sidebar.selectbox("Choose the app mode",
                                    ["Home", "Data Insight", "Forecasting",
                                        "Anomaly Detection", "My Account", "About"],
