...
class Forecast:
    ...
    def auto_arima(df, is_sessional):
        ...
        pred_df = pd.DataFrame(
            {'Prediction': pred_data_test}, index=df.index, columns=['Prediction'])
    ...
    def LSTM(df, size, loss, optimizer, epochs, look_back=1, early_stop=True):
        ...
        trainT = train.values.reshape(-1, 1)
        testT = test.values.reshape(-1, 1)
        ...
        earlyStop = tf.keras.callbacks.EarlyStopping(
            monitor="loss", verbose=1, mode='auto', patience=10)
        ...
        if (early_stop == True):
            model.fit(trainX, trainY, validation_data=(testX, testY),
                      epochs=epochs, batch_size=1, verbose=2, callbacks=[earlyStop])
        else:
            model.fit(trainX, trainY, validation_data=(testX, testY),
        ...
        trainDATA = pd.DataFrame(
            {'train_pred': trainPredict.flatten(), 'Actual': trainY.flatten()})
        testDATA = pd.DataFrame(
            {'test_pred': testPredict.flatten(), 'Actual': testY.flatten()})
    ...
    def RNN(df, train_size, window_size, loss, optimizer, epochs):
        ...
        train_set = windowed_dataset(
            x_train, window_size, batch_size=10, shuffle_buffer=10)
        ...
        pred_df_train = pd.DataFrame(
            {'train_pred': scaler.inverse_transform(x_pred).flatten()}, index=time_train)
        pred_df_test = pd.DataFrame(
            {'test_pred': scaler.inverse_transform(y_pred).flatten()}, index=time_valid)
